...
def user_logout(request):
    logout(request)
    return HttpResponseRedirect('/beerbar/login')
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if fm.is_valid():
                ...
                if user is not None:
                    login(request, user)
                    return HttpResponseRedirect('/beerbar/profile')
                else:
...
def add_to_cart(request, name, price, category):
    if request.user.is_authenticated:
            if request.method == 'POST':
                ...
                pi.save()
            if category == 'vodka':
                return HttpResponseRedirect('/beerbar/products_vodka')
            elif category == 'rum':
                return HttpResponseRedirect('/beerbar/products_rum')
            elif category == 'whisky':
                return HttpResponseRedirect('/beerbar/products_whisky')
            elif category == 'gin':
                return HttpResponseRedirect('/beerbar/products_gin')
            elif category == 'mezcal':
                return HttpResponseRedirect('/beerbar/products_mezcal')
            elif category == 'wine':
                return HttpResponseRedirect('/beerbar/products_wine')
    else:
        return HttpResponseRedirect('/beerbar/login')
...
def delete(request, id):
    if request.method == 'POST':
        ...
        pi.delete()
        return HttpResponseRedirect('/beerbar/yourorder')
